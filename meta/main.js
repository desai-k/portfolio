...
function processCommits(data) {
  return d3.groups(data, (d) => d.commit)
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: `https://github.com/desai-k/portfolio/commit/${commit}`,
        author,
        date,
        time,
        timezone,
...
function renderTooltipContent(commit) {
  ...
  document.getElementById('commit-author')
    .textContent = commit.author;

  document.getElementById('commit-lines')
    .textContent = commit.totalLines;
}
...
function renderLanguageBreakdown(selection) {
  ...
  const container =
    document.getElementById('language-breakdown');
  ...
  for (const [language, count] of breakdown) {
    ...
    container.innerHTML += `
      <dt>${language}</dt>
      <dd>${count} (${d3.format('.1%')(proportion)})</dd>
    `;
  }
}
